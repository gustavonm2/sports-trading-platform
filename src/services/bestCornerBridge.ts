...
export function calculateDynamicAPM(
  ...
  globalHomeDA: number,
  globalAwayDA: number,
  nativeStats?: { home?: any; away?: any }
): { home: APMMetrics; away: APMMetrics } {
  // Métrica padrão caso não haja série histórica suficiente
  const defaultMetrics = (globalDA: number, nativeApm?: any): APMMetrics => {
    ...
    return {
      ...
      apm10: nativeApm && nativeApm.apm10 !== undefined ? nativeApm.apm10 : roundedApm,
      apm5: nativeApm && nativeApm.apm5 !== undefined ? nativeApm.apm5 : roundedApm,
      apm3: roundedApm,
      accelerationFactor: 1.0,
      ipr: roundedApm,
      dataAge: 0,
      reliable10: nativeApm && nativeApm.apm10 !== undefined ? true : false,
      reliable5: nativeApm && nativeApm.apm5 !== undefined ? true : false,
      reliable3: false,
    };
  };
  ...
  if (!snapshots || snapshots.length === 0 || currentElapsed <= 0) {
    return {
      home: defaultMetrics(globalHomeDA, nativeStats?.home),
      away: defaultMetrics(globalAwayDA, nativeStats?.away),
    };
  }
  ...
  // dataAge usa a diferença de elapsed (minutos de jogo cobertos).
  // Se começou a ler no min 38 e agora é min 41, dataAge = 3.
  const dataAge = Math.max(0, currentSnap.elapsed - earliestSnap.elapsed);
  ...
  const calculateSideAPM = (
    ...
    const getSlidingApm = (windowSize: number): { value: number; reliable: boolean } => {
      if (sortedSnaps.length < 2) return { value: apmGlobal, reliable: false };

      const targetStartElapsed = currentSnap.elapsed - windowSize;
      const startResult = findClosestSnap(targetStartElapsed);

      if (!startResult) return { value: apmGlobal, reliable: false };

      const { snap: startSnap } = startResult;

      const timeDiff = currentSnap.elapsed - startSnap.elapsed;
      if (timeDiff <= 0) return { value: apmGlobal, reliable: false };

      const daDiff = getDA(currentSnap) - getDA(startSnap);
      return { value: Math.max(0, daDiff / timeDiff), reliable: true };
    };

    const raw10 = getApmForBlock(10);
    const raw5 = getApmForBlock(5);
    const raw3 = getSlidingApm(3);
    ...
    // Se não há bloco confiável (ex: jogo acabou de ser aberto e não há snapshots),
    // estimamos matematicamente a pressão mais recente baseada na tendência.
    const nat = nativeStats ? (isHome ? nativeStats.home : nativeStats.away) : undefined;
    const fallback5 = nat?.apm5 !== undefined ? nat.apm5 : apmGlobal;
    const fallback10 = nat?.apm10 !== undefined ? nat.apm10 : fallback5;
    
    let fallback3 = fallback5;
    if (fallback10 !== fallback5) {
      // Extrapolação matemática do ATM 3 baseada na tendência entre ATM 10 e ATM 5
      // olderApm5 = APM dos 5 minutos mais antigos dentro da janela de 10
      const olderApm5 = 2 * fallback10 - fallback5;
      const slope = (fallback5 - olderApm5) / 5;
      fallback3 = Math.max(0, fallback5 + slope * 1);
    }

    // Se bloco confiável → usar valor do bloco; senão → Fallback
    const rawApm10 = raw10.reliable ? raw10.value : fallback10;
    const rawApm5  = raw5.reliable  ? raw5.value  : fallback5;
    const rawApm3  = raw3.reliable  ? raw3.value  : fallback3;
    ...
    return {
      ...
      apm10: Math.round((nat?.apm10 !== undefined ? nat.apm10 : apm10) * 100) / 100,
      apm5: Math.round((nat?.apm5 !== undefined ? nat.apm5 : apm5) * 100) / 100,
      ...
      reliable10: nat?.apm10 !== undefined ? true : (raw10.reliable && ramp10 > 0),
      reliable5: nat?.apm5 !== undefined ? true : (raw5.reliable && ramp5 > 0),
